...
- [x] **checkbox GFM widening** (inline fix, 2026-07-16) — follow-up to T32: widened regex to full GFM task-list semantics (`*/-/+` bullets, ordered items `1./1)`, 1+ spaces, `[x]/[X]/ [ ]` states, bare boxes; blockquote-nested forms remain unsupported per spec). Hashes for legacy single-space forms unchanged; toggle writes lowercase `x`. Reverses T32's deliberate `+ [ ]` exclusion (S32.1) — user feedback showed checkboxes rendering as plain text, now GFM-robust. Backend suite 270 green, ruff clean, basedpyright 0; frontend 140 green; end-to-end headless Chromium verification all forms render/toggle correctly. Knowledge base updated: `docs/architecture.md` new checkpoint subsection documenting syntax + deliberate blockquote exclusion; no D24 amendment needed (D24 is about redaction rendering, not checkbox syntax).
