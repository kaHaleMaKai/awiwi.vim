...
- [x] T13 — server scaffold + config (2026-07-07, S13.1 sonnet) — `server/src/awiwi/` package with Settings (pydantic-settings, env `AWIWI_HOME`), PluginConfig (permissive JSON); 7 tests; pyproject.toml cleaned (deps added, dead Django/mypy blocks removed); gates all green (pytest/ruff/basedpyright); handover at `handovers/server-rewrite/T13-scaffold-config.md`; commit hash <pending>
