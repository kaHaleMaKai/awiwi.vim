...
_Updated: 2026-07-06 (later) — **T11 partially drained, then BLOCKED on a critical tooling gap**: this session ran non-interactive and `nvim` is not on the Bash allowlist (`.claude/settings.json`), so the test gate `nvim --clean --headless -l tests/run.lua` was permission-denied — no code change could land. COORD-1 reconciliation is **done** (verdict below); it surfaced one new live bug **B13** with an exact known fix, left unapplied (tree clean). **Next: finish T11 in a session that can run nvim** — see "next session needs"._
...
- **Critical tooling gap (blocked this session):** `nvim` is not in `.claude/settings.json` `permissions.allow`, and the session ran non-interactive, so `nvim --clean --headless -l tests/run.lua` was denied — the test gate cannot run. Fix: user adds `"Bash(nvim --clean --headless:*)"` (or runs T11 interactively and approves). Orchestrator did NOT self-edit settings or bypass the gate; tree left clean.
