...
_Updated: 2026-07-06 — **T10 SIGNED OFF and merged to master** after 3-round dogfood (T10.1 + T10.2 fixes, suite 458 green, 14 files). Per-module briefs archived to `handovers/done/lua-port/`, worktree removed. **Next: T11 in a fresh session** — see "next session needs"._
...
- **T11 (start here, fresh session):** drain the deferred-bugs queue above — COORD-1 reconciliation (check `handovers/done/lua-port/path.md` `## Ported` for what T2 actually did to `relativize`, then judge the hi.lua side) + PENDING-ADR `split_screen` inverted guard (needs the user's intended-behavior decision first).
